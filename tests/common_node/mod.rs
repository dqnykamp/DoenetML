...
use doenet_core::state::{EssentialStateVar, Freshness};
...
use doenet_core::{DoenetCore, state_variables::StateVarValue, state::{StateVar}};
...
fn get_state_var<'a>(dc: &'a DoenetCore, comp_name: &'static str, sv_name: &'static str) -> &'a StateVar {

    let component_ind = *dc.component_name_to_ind.get(comp_name).expect(
        &format!("Component {} does not exist", comp_name)
    );

    let component = &dc.component_nodes[component_ind];

    let state_var_ind = *component.definition.state_var_index_map.get(sv_name).expect(
        &format!("State var [{}]:[{}] does not exist", comp_name, sv_name)
    );

    let state_var = &dc.component_state_variables[component_ind][state_var_ind];
...
fn assert_state_var_is(dc: &DoenetCore, comp_name: &'static str, sv_name: &'static str, value: StateVarValue) {
    ...
    let state_var = get_state_var(dc, comp_name, sv_name);
    assert_eq!(state_var.get_freshness(), Freshness::Fresh, "Value [{}]:[{}] was stale or unresolved, expected {}", comp_name, sv_name, value);

    match value {
        StateVarValue::Number(num_val) => {

            let actual_num: f64 = if let StateVarValue::Number(actual_num) = state_var.get_fresh_value() {
                actual_num
            } else {
                panic!("Value [{}]:[{}] was {}, expected {}", comp_name, sv_name, state_var.get_fresh_value(), value);
            };
            ...
            assert!(
                ...
                "Value [{}]:[{}] was {}, expected {}", comp_name, sv_name, state_var.get_fresh_value(), value
            );

        },
        _ => {
            assert_eq!(value, state_var.get_fresh_value(), "Incorrect value from [{}]:[{}]", comp_name, sv_name);
        },
    }
}

pub fn assert_state_var_stale(dc: &DoenetCore, comp_name: &'static str, sv_name: &'static str) {

    let state_var = get_state_var(dc, comp_name, sv_name);

    assert_eq!(Freshness::Stale, state_var.get_freshness(), "Expected stale for [{}]:[{}]", comp_name, sv_name);
}

pub fn assert_state_var_unresolved(dc: &DoenetCore, comp_name: &'static str, sv_name: &'static str) {

    let state_var = get_state_var(dc, comp_name, sv_name);

    assert_eq!(Freshness::Unresolved, state_var.get_freshness(), "Expected unresolved for [{}]:[{}]", comp_name, sv_name);
}
...
pub fn assert_sv_is_number(dc: &DoenetCore, comp_name: &'static str, sv_name: &'static str, value: f64) {
    assert_state_var_is(dc, comp_name, sv_name, StateVarValue::Number(value));
}
pub fn assert_sv_is_boolean(dc: &DoenetCore, comp_name: &'static str, sv_name: &'static str, value: bool) {
    assert_state_var_is(dc, comp_name, sv_name, StateVarValue::Boolean(value));
}
pub fn assert_sv_is_integer(dc: &DoenetCore, comp_name: &'static str, sv_name: &'static str, value: i64) {
    assert_state_var_is(dc, comp_name, sv_name, StateVarValue::Integer(value));
}
...
pub fn update_immediate_value_for_text(dc: &mut DoenetCore, component_name: &'static str, value: &'static str) {
...
pub fn update_immediate_value_for_number(dc: &mut DoenetCore, component_name: &'static str, value: &'static str) {
    let type_in_number_input = Action {
        component_name: component_name.to_string(),
        action_name: "updateImmediateValue".to_string(),
        args: HashMap::from([
...
pub fn update_value_for_input(dc: &mut DoenetCore, component_name: &'static str) {
    let update_input_value = Action {
        component_name: component_name.to_string(),
        action_name: "updateValue".to_string(),
        args: HashMap::new(),
    };
    doenet_core::handle_action(dc, update_input_value);
}


pub fn move_point_2d(dc: &mut DoenetCore, component_name: &'static str, x: StateVarValue, y: StateVarValue) {
