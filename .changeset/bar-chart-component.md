...
Together with the counting operators, a whole sampling simulation is four components, none of which grows with the sample:
...
The vertical axis scales itself: one tick above the tallest bar so it never touches the frame, ticks on multiples of the step so one of them lands on zero, and the same treatment below the axis when a value is negative. `yMin` and `yMax` override it. An empty chart still draws a box one tick tall rather than collapsing, and a category nothing landed in keeps its bar at zero rather than disappearing and shifting the others along.
