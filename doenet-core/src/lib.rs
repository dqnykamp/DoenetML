...
pub struct DoenetCore {
    ...
    pub dependent_on_essential:
        HashMap<(ComponentInd, EssentialDataOrigin), Vec<ComponentState>>,
    ...
    pub mark_stale_stack: Vec<(ComponentInd, StateVarInd)>,
...
fn copy_source_for_ml_component(
    ...
    Ok(Some(CopySource::StateVar(ComponentState {
...
#[derive(Debug, Clone)]
pub struct EssentialState(ComponentInd, EssentialDataOrigin);
...
fn get_state_var_value(
    ...
    dependent_on_state_var: &mut Vec<Vec<Vec<ComponentState>>>,
    ...
    component_state_variables[original_component_state.component_ind]
        [original_component_state.state_var_ind]
        .get_fresh_value()
}
...
fn freshen_state_var(
    ...
    let current_freshness = component_state_variables[original_component_state.component_ind]
        [original_component_state.state_var_ind]
        .get_freshness();
    ...
    'stack_loop: while let Some(calculation_state) = freshen_stack.pop() {
        ...
        match calculation_state {
            StateVarCalculationState::Unresolved(unresolved_state) => {
                ...
                let component_ind = component_state.component_ind;
                let state_var_ind = component_state.state_var_ind;
                        ...
                        for dep in instruct_dependencies.iter() {
                            match dep {
                                Dependency::StateVar {
                                    ...
                                    vec_dep[*inner_sv_ind].push(ComponentState {
                                        component_ind,
                                        state_var_ind,
                                    });
                                }
                        ...
                        .skip(initial_val_ind)
                    {
                        match dep {
                            Dependency::StateVar {
                                ...
                                let new_state_var =
                                    &component_state_variables[*comp_ind_inner][*sv_ind_inner];

                                let new_current_freshness = new_state_var.get_freshness();

                                let state_var_read_only_view = match new_current_freshness {
                                    // No need to continue if the state var is already fresh
                                    Freshness::Fresh => new_state_var.get_read_only_view(),
            ...
            StateVarCalculationState::Stale(stale_state) => {
                ...
                let dependencies_for_state_var = &dependencies[component_state.component_ind]
                    [component_state.state_var_ind]
                    .dependencies;
                    ...
                    .skip(stale_state.instruction_ind)
                {
                    for (val_ind, dep) in deps.iter().enumerate().skip(initial_val_ind) {
                        ...
                        } = dep
                        {
                            ...
                            let new_state_var =
                                &component_state_variables[*component_ind][*state_var_ind];
...
fn dependencies_of_state_var<'a>(
    ...
    let my_deps =
        &dependencies[component_state.component_ind][component_state.state_var_ind].dependencies;
...
fn mark_stale_state_var_and_dependencies(
    dependent_on_state_var: &Vec<Vec<Vec<ComponentState>>>,
    component_state_variables: &mut Vec<Vec<StateVar>>,
    stale_renderers: &mut HashSet<ComponentInd>,
    mark_stale_stack: &mut Vec<(ComponentInd, StateVarInd)>,
    original_component_ind: ComponentInd,
    original_state_var_ind: StateVarInd,
) {
    mark_stale_stack.push((original_component_ind, original_state_var_ind));

    while let Some((component_ind, state_var_ind)) = mark_stale_stack.pop() {
        ...
        if state.get_freshness() == Freshness::Fresh {
            ...
            {
                mark_stale_stack.push((*new_comp_ind, *new_sv_ind));
            }
        }
    }
}
...
fn mark_stale_essential_datum_dependencies(
    dependent_on_state_var: &Vec<Vec<Vec<ComponentState>>>,
    dependent_on_essential: &HashMap<(ComponentInd, EssentialDataOrigin), Vec<ComponentState>>,
    component_state_variables: &mut Vec<Vec<StateVar>>,
...
fn generate_render_tree_internal(
    ...
    for state_var_ind in renderered_state_var_inds {
        let component_state = ComponentState {
            component_ind: component.component_node.ind,
            state_var_ind,
        };
        freshen_state_var(
            component_name_to_ind,
...
pub fn handle_action(core: &mut DoenetCore, action: Action) {
    ...
    for (state_var_ind, requested_value) in state_vars_to_update {
        ...
        process_update_request(
            ...
            &mut core.mark_stale_stack,
            &mut core.update_stack,
            request,
        );
    }
...
fn convert_dependency_values_to_update_request<'a, 'b>(
    ...
    } in requests
    {
        let instruct_dependencies = my_dependencies.get(instruction_ind).expect(&format!(
            ...
            component_nodes[component_ind].definition.component_type, state_var_ind
        ));
...
fn process_update_request(
    ...
    stale_renderers: &mut HashSet<ComponentInd>,
    mark_stale_stack: &mut Vec<(ComponentInd, StateVarInd)>,
    update_stack: &mut Vec<UpdateRequest>,
...
fn request_dependencies_to_update_value_including_shadow<'a, 'b>(
    ...
    let component_ind = component_state.component_ind;
    let state_var_ind = component_state.state_var_ind;
    let state_variable = &component_state_variables[component_ind][state_var_ind];

    if let Some(source_component_ref_state) =
        state_var_is_shadowing(component_nodes, component_state)
    {
        // TODO: state variable shadowing needs to be updated to state variable traits

        let source_state_var_ind = source_component_ref_state.state_var_ind;

        let source_state_var = &component_state_variables[source_component_ref_state.component_ind]
            [source_state_var_ind];
        ...
        vec![UpdateRequest::SetStateVar(source_component_ref_state)]
    } else {
