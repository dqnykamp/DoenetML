...
export default class Spreadsheet extends BlockComponent {
    ...
    static createAttributesObject() {
        ...
        attributes.fixedColumnsLeft = {
            description: "Number of left columns fixed in place when scrolling.",
            createComponentOfType: "integer",
    ...
    static returnStateVariableDefinitions() {
        ...
        stateVariableDefinitions.height = {
            ...
            definition({ dependencyValues }) {
                if (dependencyValues.heightAttr === null) {
                    ...
                    return {
                        setValue: {
                            height: { size: height, isAbsolute: true },
                        },
                    };
                }
                ...
                return {
                    setValue: {
                        height: dependencyValues.heightAttr.stateValues
                            .componentSize,
                    },
                };
            },
        ...
        stateVariableDefinitions.cells = {
            ...
            arrayDefinitionByKey({
                ...
                for (let arrayKey of arrayKeys) {
                    ...
                    if (
                        dependencyValuesByKey[arrayKey].cellText !== undefined
                    ) {
                        cells[arrayKey] =
                            dependencyValuesByKey[arrayKey].cellText;
                    } else if (
                        rowInd < dataFrameNumRows &&
                        colInd < dataFrameNumColumns
                    ) {
                        cells[arrayKey] =
                            dataFrame.stateValues.dataFrame.data[rowInd][
                                colInd
                            ];
                    } else {
            ...
            inverseArrayDefinitionByKey({
                ...
                for (let arrayKey in desiredStateVariableValues.cells) {
                    if (dependencyNamesByKey[arrayKey].cellText !== undefined) {
                        instructions.push({
                            setDependency:
                                dependencyNamesByKey[arrayKey].cellText,
                            desiredValue:
                                desiredStateVariableValues.cells[arrayKey],
                        });
                    } else {
                        let cellText =
                            desiredStateVariableValues.cells[arrayKey];
        ...
        stateVariableDefinitions.evaluatedCells = {
            ...
            arrayVarNameFromPropIndex(propIndex, varName) {
                if (
                    varName === "evaluatedCells" ||
                    varName === "evaluatedRows"
                ) {
                    if (propIndex.length === 1) {
