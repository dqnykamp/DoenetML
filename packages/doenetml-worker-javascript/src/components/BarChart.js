...
import { returnGraphHasAuthorAnnotationsStateVariableDefinition } from "../utils/prefigure/stateVariable";
import {
 ...
 * many there are, the horizontal axis can carry category *names* (JSXGraph
 * gives Doenet no custom-tick support at all), and the result is navigable by
...
export default class BarChart extends BlockComponent {
    ...
    static createAttributesObject() {
        ...
        attributes.yMin = {
            description:
                "Lowest value shown on the vertical axis. Defaults to 0, or to the smallest value when some are negative.",
            createComponentOfType: "number",
        ...
        attributes.yMax = {
            description:
                "Highest value shown on the vertical axis. Defaults to the next tick above the tallest bar.",
            createComponentOfType: "number",
    ...
    static returnStateVariableDefinitions() {
        ...
        // with nothing PreFigure-specific in it.
        stateVariableDefinitions.chartGeometry = {
            ...
            definition({ dependencyValues }) {
                const diagnostics = [];
                let barWidth = dependencyValues.barWidth;
                if (!(barWidth > 0) || barWidth > 1) {
                    diagnostics.push(
                        codedDiagnostic({
                            type: "warning",
                            code: "doenet-w0142",
                            args: { barWidth: String(barWidth) },
                        }),
                    );
                    barWidth = 0.8;
                }
        ...
        stateVariableDefinitions.prefigureXML = {
            ...
            definition({ dependencyValues }) {
                ...
                const aspectRatio =
                    dependencyValues.aspectRatio > 0
                        ? dependencyValues.aspectRatio
                        : 1.5;
