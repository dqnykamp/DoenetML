    ...
    static returnStateVariableDefinitions(numerics) {
        ...
        stateVariableDefinitions.numericalThroughPoints = {
            ...
            arrayDefinitionByKey({ dependencyValuesByKey, arrayKeys }) {
                ...
                for (let arrayKey of arrayKeys) {
                    let point = dependencyValuesByKey[
                        arrayKey
                    ].throughPoint.map((x) => x.tree);
            ...
            inverseArrayDefinitionByKey({
                ...
                for (let arrayKey in desiredStateVariableValues.numericalThroughPoints) {
                    if (dependencyValuesByKey[arrayKey].throughPoint) {
                        instructions.push({
                            setDependency:
                                dependencyNamesByKey[arrayKey].throughPoint,
                            desiredValue:
        ...
        stateVariableDefinitions.a = {
            public: true,
            description: "The leading coefficient of the parabola (in y = a x^2 + b x + c).",
            isLocation: true,
            shadowingInstructions: {
                createComponentOfType: "number",
                addAttributeComponentsShadowingStateVariables:
            ...
            definition: function ({ dependencyValues, componentIdx }) {
                ...
                if (dependencyValues.numericalThroughPoints.length === 0) {
                    ...
                    dependencyValues.numericalThroughPoints.length === 2
                ) {
                    // two points
                    ...
                    dependencyValues.numericalThroughPoints.length === 3
                ) {
                    // three points
        ...
        stateVariableDefinitions.vertex = {
            ...
            inverseArrayDefinitionByKey: async function ({
                ...
                if ("0" in desiredStateVariableValues.vertex) {
                    x =
                        desiredStateVariableValues.vertex[0].evaluate_to_constant();
                ...
                if ("1" in desiredStateVariableValues.vertex) {
                    y =
                        desiredStateVariableValues.vertex[1].evaluate_to_constant();
                ...
                return {
                    setValue: {
                        nearestPoint: function ({ variables, scales }) {
                            ...
                            let a0 =
                                (dependencyValues.a * xscale * xscale) / yscale;
                            ...
                            if (a0 === 0) {
                                ...
                                let result = {};
                                result.x1 =
                                    ((x1 + b0 * x2 - b0 * c0) / denom) * xscale;
                                result.x2 =
                                    ((b0 * (x1 + b0 * x2) + c0) / denom) *
                                    yscale;
                            ...
                            let x2AtMin =
                                dependencyValues.f(x1AtMin * xscale) / yscale;
                            let d2AtMin =
                                (x1 - x1AtMin) ** 2 + (x2 - x2AtMin) ** 2;

                            for (let r of resultCardano.slice(1)) {
                                let x = r;
                                let fx =
                                    dependencyValues.f(x * xscale) / yscale;
