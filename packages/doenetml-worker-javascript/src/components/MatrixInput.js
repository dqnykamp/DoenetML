...
export class MatrixInput extends Input {
    ...
    static componentDocs = {
        summary: "An interactive matrix input where each cell accepts a math value.",
    };
    ...
    static returnStateVariableDefinitions() {
        ...
        stateVariableDefinitions.valueChanged = {
            description: "Whether the saved matrix has been changed from its initial state.",
            public: true,
            hasEssential: true,
            defaultValue: false,
        ...
        stateVariableDefinitions.immediateValueChanged = {
            description: "Whether the live matrix differs from its initial state.",
            public: true,
            hasEssential: true,
            defaultValue: false,
            shadowingInstructions: {
                    ...
                    return {
                        setValue: {
                            immediateValueOriginal:
                                dependencyValues.valueOriginal,
                        },
                        setEssentialValue: {
                            immediateValueOriginal:
                                dependencyValues.valueOriginal,
            ...
            inverseDefinition: function ({
                ...
                if (!(initialChange || shadowedVariable)) {
                    instructions.push({
                        ...
                        desiredValue:
                            desiredStateVariableValues.immediateValueOriginal,
                    });
                }
        ...
        stateVariableDefinitions.numRows = {
            ...
            definition({ dependencyValues, usedDefault }) {
                ...
                if (
                    usedDefault.numRowsPreliminary ||
            ...
            async inverseDefinition({
                ...
                if (dependencyValues.haveBoundValue) {
                    let originalTree = dependencyValues.valueOriginal.tree;
                    let defaultEntryTree = (await stateValues.defaultEntry)
                        .tree;
                    if (
            ...
            definition({ dependencyValues, previousValues }) {
                ...
                if (Array.isArray(originalTree)) {
                    ...
                    if (operator === "matrix") {
                        ...
                        for (let [rowInd, comp] of originalTree
                            .slice(1)
                            .entries()) {
                            let accumRow = accumulatedComponents[rowInd];
                            if (!accumRow) {
                        ...
                        for (let [colInd, comp] of originalTree[1]
                            .slice(1)
                            .entries()) {
                            accumRow[colInd] = comp;
                        }
        ...
        stateVariableDefinitions.componentValues = {
            ...
            arrayDefinitionByKey({ globalDependencyValues, arrayKeys }) {
                ...
                if (Array.isArray(originalTree)) {
                    ...
                    if (operator === "matrix") {
                        ...
                        for (
                            let rowInd = 0;
                            rowInd < numOverlapRows;
                            rowInd++
                        ) {
                            for (
                                let colInd = 0;
                                colInd < numOverlapColumns;
                                colInd++
                            ) {
                                let arrayKey = `${rowInd},${colInd}`;
                                componentValues[arrayKey] = me.fromAst(
            ...
            inverseArrayDefinitionByKey({
                ...
                if (workspace.valueData) {
                    ...
                    let originalTree =
                        globalDependencyValues.valueOriginal.tree;
        ...
        stateVariableDefinitions.componentImmediateValues = {
            ...
            arrayDefinitionByKey({ globalDependencyValues, arrayKeys }) {
                let originalTree =
                    globalDependencyValues.immediateValueOriginal.tree;
                let accumulatedComponents =
                    globalDependencyValues.accumulatedComponents;
            ...
            inverseArrayDefinitionByKey({
                ...
                if (workspace.immediateValueData) {
                    ...
                    if (originalIsColumnVector) {
                        ...
                        immediateValueData = [
                            originalTree[1].slice(1, 1 + numColumns),
                        ];
                ...
                for (let arrayKey in desiredStateVariableValues.componentImmediateValues) {
                    let [rowInd, colInd] = arrayKey.split(",");
                    immediateValueData[rowInd][colInd] =
                        convertValueToMathExpression(
                ...
                if (numColumns === 1 && originalIsColumnVector) {
                    let operator =
                        globalDependencyValues.immediateValueOriginal.tree[0];
                    let desiredValue = me.fromAst([
                        operator,
                    ...
                    let desiredValueTree = [
                        ...
                        [
                            "tuple",
                            ...immediateValueData.map((x) => ["tuple", ...x]),
                        ],
                    ];
        ...
        stateVariableDefinitions.componentValuesForDisplay = {
            ...
            returnArrayDependenciesByKey({ arrayKeys }) {
                ...
                for (let arrayKey of arrayKeys) {
                    let [rowInd, colInd] = arrayKey.split(",");
                    let varEnding =
                        Number(rowInd) + 1 + "_" + (Number(colInd) + 1);
            ...
            definition({ dependencyValues }) {
                ...
                for (let rowInd = 0; rowInd < numRows; rowInd++) {
                    ...
                    for (let colInd = 0; colInd < numColumns; colInd++) {
                        data[rowInd + 1][colInd + 1] =
                            dependencyValues.componentValues[rowInd][
                                colInd
                            ].tree;
                    }
                }
                ...
                if (Array.isArray(desiredTree) && desiredTree[0] === "matrix") {
                    if (
                        ...
                        if (dependencyValues.numColumns === 1) {
                            let originalTree =
                                dependencyValues.valueOriginal.tree;
                            if (Array.isArray(originalTree)) {
                                let operator = originalTree[0];
                                if (vectorOperators.includes(operator)) {
        ...
        stateVariableDefinitions.immediateValue = {
            description: "The current matrix being entered (live, before saving).",
            public: true,
            shadowingInstructions: {
                createComponentOfType: "math",
                addAttributeComponentsShadowingStateVariables:
            ...
            definition({ dependencyValues }) {
                ...
                for (let rowInd = 0; rowInd < numRows; rowInd++) {
                    ...
                    for (let colInd = 0; colInd < numColumns; colInd++) {
                        data[rowInd + 1][colInd + 1] =
                            dependencyValues.componentImmediateValues[rowInd][
                                colInd
                            ].tree;
                    }
                }
            ...
            inverseDefinition({
                ...
                if (Array.isArray(desiredTree) && desiredTree[0] === "matrix") {
                    if (
                        ...
                        if (dependencyValues.numColumns === 1) {
                            let originalTree =
                                dependencyValues.immediateValueOriginal.tree;
                            if (Array.isArray(originalTree)) {
                                let operator = originalTree[0];
                                if (vectorOperators.includes(operator)) {
        ...
        stateVariableDefinitions.focused = {
            description: "Whether the matrix input currently has keyboard focus.",
            public: true,
            shadowingInstructions: {
                createComponentOfType: "boolean",
            },
...
export class MatrixInputRow extends CompositeComponent {
    ...
    static async calculateReplacementChanges({
        ...
        if (numColumns < previousNumColumns) {
            newReplacementsToWithhold =
                component.replacements.length - numColumns;
    ...
    static returnStateVariableDefinitions() {
        ...
        stateVariableDefinitions.value = {
            ...
            returnDependencies: ({ stateValues }) => {
                let varEnding = "";
                if (
                    stateValues.rowInd !== null &&
                    stateValues.colInd !== null
                ) {
                    varEnding = `${stateValues.rowInd + 1}_${
                        stateValues.colInd + 1
                    }`;
                }
                return {
            ...
            definition: function ({ dependencyValues }) {
                ...
                if (dependencyValues.parentComponentValue) {
                    return {
                        setValue: {
                            value: dependencyValues.parentComponentValue,
                        },
                    };
                } else {
