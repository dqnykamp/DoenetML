 ...
 *
 * A `NaN` is no category. It reaches a numerically compared list because
...
function distinctValues(values, numeric) {
    ...
    for (const value of values) {
        if (numeric && Number.isNaN(value.numericalValue)) {
            continue;
        }
        if (
            !distinct.some(
...
function tallyValues({ values, numeric, declaredCategories, declaredType }) {
    ...
    for (const value of values) {
        const ind = comparableCategories.findIndex(
